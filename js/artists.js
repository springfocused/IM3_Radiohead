...
const topArtist = document.getElementById('topArtist'); // Container für Top-Songs abrufen
...
async function fetchData() {
    ...
    try {
        ...
        topArtist.innerHTML = `<p>Fehler beim Laden der Daten.</p>`;
    }
}
...
function displayData(data) {
    ...
    if (data.length === 0) {
        topArtist.innerHTML = '<p>Keine Daten gefunden.</p>';
        return;
    }
...
const limitedData = data.slice(0, 20);
    ...
    limitedData.forEach((item, index) => {
        ...
        titleElement.classList.add('artist-item');
        
        // HTML-Struktur erstellen, die den Titel und die Wiedergabezahlen enthält
        titleElement.innerHTML = `
            <div class="artist-rank"><p>${index + 1}</p></div>
            <div class="artist"><p>${item.artist}</p></div>
            <div class="artist-plays"><p>${item.play_count}</p></div>
        `;
        // Füge das erstellte Element in den Top Songs-Container ein
        topArtist.appendChild(titleElement);
    });
}
